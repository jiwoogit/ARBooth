...
class Args(Tap):
    #############################################################################################
    ######################### Training Options related to our paper ##########################
    #############################################################################################
    enable_scale_weighted_loss: int = 1 # 0: no scale weighted cross-entropy loss, 1: scale weighted cross-entropy loss
    train_iter: int = 500
    enable_sa_layer: int = 0 # 0: no self-attention layer, 1: self-attention layer / opts for selective layer tuning
    enable_ca_layer: int = 1 # 0: no cross-attention layer, 1: cross-attention layer / opts for selective layer tuning
    enable_ffn_layer: int = 1 # 0: no FFN layer, 1: FFN layer / opts for selective layer tuning
    enable_all_layer: int = 0 # 0: no all layer, 1: all layer / opts for selective layer tuning
    enable_lora: int = 0 # 0: no lora, 1: lora / not supported yet
    lora_rank: int = 4
    reg_bit_flip: int = 0
    reg_data_path: str = ''
    distill_type: str = 'soft' # 'hard': hard distillation, 'soft': soft distillation
    distill_temperature: float = 1.0 # use only soft distillation
    text_prefix_length: int = 4 # length of S* text embedding (2~4 is recommended)
    reg_lambda: float = 0.5 # lambda for regularization
    instance_prompt: str = '<placeholder> dog' # instance prompt
    regularize_prompt: str = '' # regularize prompt
    class_prompt: str = 'dog' # class prompt
    text_prefix_prompt: str = 'a photo of'
    val_prompt_path: str = 'inputs/prompts_validate.txt'
    with_reg: int = 1 # 0: no prior distillation, 1: prior distillation
    bf16: int = 1
    wandb: int = 0 # 0: no wandb, 1: wandb
    cfg_insertion_layer: int = 0
    center_crop: bool = True # whether to use center crop
    finetuning_batch_size: int = 2 # batch size for finetuning
    model_path: str = 'weights/infinity_2b_reg.pth' # path to the model checkpoint
    vae_path: str = 'weights/infinity_vae_d32_reg.pth' # path to the VAE checkpoint
    text_encoder_ckpt: str = 'google/flan-t5-xl' # path to the text encoder checkpoint
    #############################################################################################
    text_channels: int = 2048 # number of text channels
    ...
    vae_ckpt: str = 'weights/infinity_vae_d32_rdn_short.pth'                  # VAE ckpt
    exp_name: str = ''                  # experiment name
    ds: str = 'oi'                      # only used in GPT training::load_viz_data & FID benchmark
    model: str = '2bc8'                     # for VAE training, 'b' or any other for GPT training
    ...
    # dir
    log_txt_path: str = ''
    t5_path: str = 'google/flan-t5-xl'                   # if not specified: automatically find from all bytenas
    ...
    tau: float = 0.5                      # tau of self attention in GPT
    ...
    cfg: float = 4                    # >0: classifier-free guidance, drop cond with prob cfg
    ...
    wp: float = 0.00000001
    wp0: float = 0.005
    wpe: float = 1.0                    # 0.001, final cosine lr = wpe * peak lr
    ...
    lbs: int = 4                        # local batch size; if lbs != 0, bs will be ignored, and will be reset as round(args.lbs / args.ac) * dist.get_world_size()
    ...
    save_model_iters_freq: int = 100   # save model iter freq
    save_txt_iters_freq: int = 100      # save model iter freq
...
def init_dist_and_get_args():
    ...
    args.instance_prompt = args.text_prefix_prompt + ' ' + args.instance_prompt
    args.regularize_prompt = args.text_prefix_prompt + ' ' + args.class_prompt
