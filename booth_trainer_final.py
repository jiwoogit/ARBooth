...
class InfinityTrainer(object):
    def __init__(
        ...
        vae_local, gpt_wo_ddp: Infinity, gpt: DDP, gpt_ori: DDP, ema_ratio: float, max_it: int,
        gpt_opt: AmpOptimizer, txt_opt: AmpOptimizer, label_smooth: float, z_loss_ratio: float, eq_loss: int, xen: bool,
        dbg_unused=False,zero=0, vae_type=True,
        ...
        self.gpt_ori = gpt_ori
        self.quantize_local: VectorQuantizer2
        self.gpt_opt: AmpOptimizer = gpt_opt
        self.txt_opt: AmpOptimizer = txt_opt
    ...
    def eval_ep(self, ep: int, args: arg_util.Args, ld_val: DataLoader):
        ...
        for inp, label_B in ld_val:
            ...
            self.gpt.forward
            logits_BLV = self.gpt(label_B, self.quantize_local.fuse_multiscale_idx_as_gpt_inp_BL(gt_ms_idx_Bl))
    ...
    def train_step(
        ...
        inp_B3HW: FTen, cls_inp_B3HW: FTen, text_cond_tuple: Union[ITen, FTen], reg_text_cond_tuple: Union[ITen, FTen], args: arg_util.Args,
        ...
        # print(scale_schedule) # [(1, 1, 1), (1, 2, 2), (1, 4, 4), (1, 6, 6), (1, 8, 8), (1, 12, 12), (1, 16, 16)]
        
        # [forward]
        with self.txt_opt.amp_ctx:
            ...
            if args.enable_scale_weighted_loss == 1:
                training_scales_ = 8
                scaling_factor = torch.ones_like(loss, device=loss.device)
                training_seq_len_ = np.array(scale_schedule)[:training_scales_].prod(axis=1).sum()
                scaling_factor[:, training_seq_len_:] *= 0.5
                loss *= scaling_factor
        ...
        # [backward]
        grad_norm_t, scale_log2_t = self.txt_opt.backward_clip_step(ep=ep, it=it, g_it=g_it, stepping=stepping, logging_params=logging_params, loss=loss, clip_decay_ratio=clip_decay_ratio, stable=args.stable)
        ...
        if stepping:
            ...
            if self.dbg_unused:
                ls = []
                for n, p in self.gpt.named_parameters():
                    if p.grad is None:
                        ls.append(n)
                if len(ls):
                    raise AttributeError(f'unused param: {ls}')
        
            self.txt_opt.optimizer.zero_grad(set_to_none=True)
        ...
        if args.with_reg:
            with self.txt_opt.amp_ctx:
                ...
                with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16, cache_enabled=False):
                    ...
                    class_BLC, ori_logits_BLV, cond_BD, cond_BD_or_gss, ca_kv = self.gpt_ori.autoregressive_cls_infer(
                        self.vae_local, reg_text_cond_tuple,
                        scale_schedule=scale_schedule[:training_scales],
                        cfg_list=cfg_list, tau_list=tau_list, B=B, top_k=900, top_p=0.97,
                    )

                    reg_logits_BLV = self.gpt(reg_text_cond_tuple, class_BLC, scale_schedule=scale_schedule[:training_scales],
                        cond_BD=cond_BD, cond_BD_or_gss=cond_BD_or_gss, ca_kv=ca_kv
                    )
                
                if args.use_bit_label:
                    reg_gt_BL = ori_logits_BLV.reshape(tmp_bs, tmp_seq_len, -1, 2).argmax(dim=-1)  # shape: [bs, seq_len, num_tokens]
                    if args.distill_type == "soft":
                        temp = args.distill_temperature
                        teacher_probs = torch.softmax(ori_logits_BLV / temp, dim=-1)
                        student_log_probs = torch.log_softmax(reg_logits_BLV / temp, dim=-1)
                        reg_loss = F.kl_div(student_log_probs, teacher_probs, reduction='none').sum(dim=-1)

                    elif args.distill_type == "hard":
                        tmp_bs, tmp_seq_len, tmp_channel = reg_logits_BLV.shape
                        reg_loss = self.train_loss(reg_logits_BLV.reshape(tmp_bs, tmp_seq_len, -1, 2).permute(0,3,1,2), reg_gt_BL)
                        if args.bitloss_type == 'mean':
                    ...
                    reg_loss = self.train_loss(reg_logits_BLV.reshape(-1, V), reg_gt_BL).reshape(B, -1)

                reg_loss = reg_loss.mul(lw).sum(dim=-1).mean() * args.reg_lambda
        else:
            reg_loss = torch.tensor(0.0, device=loss.device, requires_grad=True)
            reg_gt_BL = gt_BL.clone().detach()
            reg_logits_BLV = logits_BLV.clone().detach()
        
        # [backward]
        reg_grad_norm_t, reg_scale_log2_t = self.txt_opt.backward_clip_step(
            ep=ep, it=it, g_it=g_it, stepping=stepping, logging_params=logging_params,
            loss=reg_loss, clip_decay_ratio=clip_decay_ratio, stable=args.stable)
        ...
        if metric_lg.log_every_iter or it == 0 or it in metric_lg.log_iters:
            ...
            if args.use_bit_label:
                res_loss = self.train_loss(
                    logits_BLV.reshape(B, seq_len, -1, 2).permute(0, 3, 1, 2), gt_BL
                ).mean(dim=-1).mean(0)
                reg_res_loss = self.train_loss(
                    reg_logits_BLV.reshape(B, seq_len, -1, 2).permute(0, 3, 1, 2), reg_gt_BL
                ).mean(dim=-1).mean(0)
                bitwise_acc = (logits_BLV.reshape(B, seq_len, -1, 2).argmax(dim=-1) == gt_BL).float()
                reg_bitwise_acc = (reg_logits_BLV.reshape(B, seq_len, -1, 2).argmax(dim=-1) == reg_gt_BL).float()
            else:
                res_loss = self.train_loss(
                    logits_BLV.reshape(-1, V), gt_BL.reshape(-1)
                ).reshape(B, -1).mean(0)
            ...
            reg_res_bit_acc = reg_bitwise_acc.mean(-1).mean(0)
            reg_res_token_acc = (reg_bitwise_acc.sum(-1) == self.vae_local.codebook_dim).float().mean(0)

            loss_token_mean = res_loss.mean().item()
            acc_bit_mean = res_bit_acc.mean().item() * 100.
            acc_token_mean = res_token_acc.mean().item() * 100.

            reg_loss_token_mean = reg_res_loss.mean().item()
            reg_acc_bit_mean = reg_res_bit_acc.mean().item() * 100.
            reg_acc_token_mean = reg_res_token_acc.mean().item() * 100.

            ptr = 0
            L_list, acc_bit_list, acc_token_list = [], [], []
            reg_L_list, reg_acc_bit_list, reg_acc_token_list = [], [], []
            for scale_ind in range(min(training_scales, len(scale_schedule))):
                area = np.array(scale_schedule[scale_ind]).prod()
                start, end = ptr, ptr + area
                ...
                reg_L_list.append(reg_res_loss[start:end].mean().item())
                reg_acc_bit_list.append(reg_res_bit_acc[start:end].mean().item() * 100.)
                reg_acc_token_list.append(reg_res_token_acc[start:end].mean().item() * 100.)

                ptr = end

            if grad_norm_t is not None:
                metrics = torch.tensor(
                    L_list + acc_bit_list + acc_token_list + [grad_norm_t.item(), loss_token_mean, acc_bit_mean, acc_token_mean],
                    device=loss.device
                )
            else:
                metrics = torch.tensor(
                    L_list + acc_bit_list + acc_token_list + [loss_token_mean, acc_bit_mean, acc_token_mean],
                    device=loss.device
                )
            metrics = metrics.cpu().data.numpy() / dist.get_world_size()
            leng = len(L_list)
            L_list, acc_bit_list, acc_token_list, grad_norm_t, loss_token_mean, acc_bit_mean, acc_token_mean = \
                metrics[:leng], metrics[leng:2*leng], metrics[2*leng:3*leng], metrics[-4], metrics[-3], metrics[-2], metrics[-1]
            ...
            metric_lg.update(
                ...
                reg_Lm=reg_loss_token_mean, reg_Lt=reg_L_list[-1],
                reg_Accm=reg_acc_bit_mean, reg_Acct=reg_acc_token_mean
            )
            ...
            if args.wandb:
                wandb_log_dict = {
                    ...
                    "reg/Overall/L_mean": reg_loss_token_mean,
                    "reg/Overall/Acc_bit_mean": reg_acc_bit_mean,
                    "reg/Overall/Acc_token_mean": reg_acc_token_mean,
                }
                ...
                for si, (loss_si, acc_bit_si, acc_token_si) in enumerate(zip(reg_L_list, reg_acc_bit_list, reg_acc_token_list)):
                    wandb_log_dict[f'reg/Detail/L_s{si+1:02d}'] = loss_si
                    wandb_log_dict[f'reg/Detail/Acc_bit_s{si+1:02d}'] = acc_bit_si
                    wandb_log_dict[f'reg/Detail/Acc_token_s{si+1:02d}'] = acc_token_si
