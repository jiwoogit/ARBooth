...
def load_transformer(vae, args, concept):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model_folder = os.path.join(args.model_path, concept)
...
def add_common_arguments(parser):
    ...
    parser.add_argument('--prefix_embeds_length', type=int, default=0)
    parser.add_argument('--text_prefix_length', type=int, default=4)
    parser.add_argument('--vae_path', type=str, default='weights/infinity_vae_d32_reg.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='normal', choices=['normal', 'challenging'])
    ...
    # add special token and resize text encoder
    ...
    vico_concepts = [
        ...
        ("fat_dog", "dog", True),
        ####################
        ("cat_statue", "statue", False),
        ("elephant_statue", "statue", False),
        ("duck_toy", "toy", False),
        ("monster_toy", "toy", False),
        ("brown_teddybear", "teddybear", False),
        ####################
        ("tortoise_plushy", "plushy", False),
        ("alarm_clock", "clock", False),
        ("pink_sunglasses", "sunglasses", False),
        ("red_teapot", "teapot", False),
        ("red_vase", "vase", False),
        ("wooden_barn", "barn", False),
        ###################
    ]
    for concept, concept_cls_name, is_live in vico_concepts:
        if is_live:
            ...
            with open("./inputs/prompts_nonlive_objects.txt", "r") as fin:
                prompts = fin.read()
                prompts = prompts.split("\n")
        infinity = load_transformer(vae, args, concept)
            ...
            with torch.no_grad():
                for sampling_idx in range(args.num_samples):
                    generated_image = gen_one_img(
                        ...
                        text_tokenizer,
                        text_encoder,
                        "a photo of {}".format(concept_cls_name),
                        g_seed=args.seed + sampling_idx,
                        gt_leak=0,
                        gt_ls_Bl=None,
                        cfg_list=args.cfg,
                        tau_list=args.tau,
                        ...
                        special_token=infinity.special_token,
                    )
                    out_path = os.path.join(
                        out_dir,
                        f"{sampling_idx:03d}.jpg"
                    )
                    os.makedirs(osp.dirname(out_path), exist_ok=True)
                    cv2.imwrite(out_path, generated_image.cpu().numpy())
                    print(f'Save to {osp.abspath(out_path)}')
